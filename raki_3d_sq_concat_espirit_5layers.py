...
def conv3d_dilate(x, W, Rx, Ry):
    return tf.nn.convolution(x, W,padding='VALID',dilation_rate = [Rx, Ry, 1])
...
def learning(ACS,target_input,Rx, Ry,sess, ACS_dim_X, ACS_dim_Y, ACS_dim_T, ACS_dim_Z, target_dim_X,target_dim_Y,target_dim_T,target_dim_Z, target, kernel_x_1, kernel_x_2, kernel_x_3, kernel_x_4, kernel_y_1, kernel_y_2, kernel_y_3, kernel_y_4, kernel_t_1, kernel_t_2, kernel_t_3, kernel_t_4, layer1_channels, layer2_channels, layer3_channels, layer4_channels, kernel_last_x, kernel_last_y, kernel_last_t, LearningRate, MaxIteration, finetune, slice_n):
    ...
    W_conv1 = weight_variable([kernel_x_1, kernel_y_1, kernel_t_1, ACS_dim_Z, layer1_channels],'W1')
    #h_conv1 = tf.nn.relu(conv3d_dilate(Input, W_conv1, Rx, Ry))
    h_conv1 = conv3d_dilate(Input, W_conv1, Rx, Ry)
    ...
    h_conv2 = conv3d_dilate(h_conv1, W_conv2, Rx, Ry)
    ...
    h_conv3 = conv3d_dilate(h_conv2, W_conv3, Rx, Ry)

    W_conv4 = weight_variable([kernel_x_4, kernel_y_4, kernel_t_4, layer3_channels, layer4_channels],'W4')
    h_conv4 = tf.nn.relu(conv3d_dilate(h_conv3, W_conv4, Rx, Ry))
    ...
    h_conv5 = conv3d_dilate(h_conv4, W_conv5, Rx, Ry)
...
def cnn_3layer(input_kspace,w1,b1,w2,b2,w3,b3,w4,w5,Rx,Ry,sess):
    ...
    h_conv1 = conv3d_dilate(input_kspace, w1, Rx,Ry)
    h_conv2 = conv3d_dilate(h_conv1, w2, Rx,Ry)
    #h_conv2 = tf.nn.relu(conv3d_dilate(h_conv1, w2, Rx,Ry))
    #h_conv3 = tf.nn.relu(conv3d_dilate(h_conv2, w3,Rx,Ry))
    h_conv3 = conv3d_dilate(h_conv2, w3, Rx,Ry)

    h_conv4 = tf.nn.relu(conv3d_dilate(h_conv3, w4, Rx,Ry))
    #h_conv4 = conv3d_dilate(h_conv3, w4, Rx,Ry)
    h_conv5 = conv3d_dilate(h_conv4, w5, Rx,Ry)
...
def raki(num_chan, N1, N2, Nt, acs_start_index_x, acs_end_index_x, acs_start_index_y, acs_end_index_y, acs_start_index_t, acs_end_index_t, kspace_undersampled_zero_filled, kspace_acs_zero_filled, kspace_coils_acs_out, kspace_acs_cropped, Rx, Ry, debug_path, name_image, name_weight,train_flag=True, finetune=False, slice_n=0):
    ...
    target_x_start = np.int32((np.ceil(kernel_x_1/2)-1) + (np.ceil(kernel_x_2/2)-1) + (np.ceil(kernel_x_3/2)-1) + (np.ceil(kernel_x_4/2)-1) + (np.ceil(kernel_last_x/2)-1)) * Rx;
    target_x_end = ACS_out_dim_X  - np.int32((np.floor(kernel_x_1/2) + np.floor(kernel_x_2/2) + np.floor(kernel_x_3/2) + np.floor(kernel_x_4/2) + np.floor(kernel_last_x/2))) * Rx -1;

    target_y_start = np.int32((np.ceil(kernel_y_1/2)-1) + (np.ceil(kernel_y_2/2)-1) + (np.ceil(kernel_y_3/2)-1) + (np.ceil(kernel_y_4/2)-1) + (np.ceil(kernel_last_y/2)-1)) * Ry;
    target_y_end = ACS_out_dim_Y  - np.int32((np.floor(kernel_y_1/2) + np.floor(kernel_y_2/2) + np.floor(kernel_y_3/2) + np.floor(kernel_y_4/2) + np.floor(kernel_last_y/2))) * Ry -1;
    ...
    target_dim_Z = Rx * Ry * ACS_out_dim_Z
    ...
    if train_flag:
        ...
        for ind_c in range(ACS_out_dim_Z):
            ...
            for ind_acc_x in range(Rx):
                ...
                target_x_start = np.int32((np.ceil(kernel_x_1/2)-1) + (np.ceil(kernel_x_2/2)-1) + (np.ceil(kernel_x_3/2)-1) + (np.ceil(kernel_x_4/2)-1) + (np.ceil(kernel_last_x/2)-1)) * Rx + ind_acc_x;
                target_x_end = ACS_out_dim_X  - np.int32((np.floor(kernel_x_1/2) + np.floor(kernel_x_2/2) + np.floor(kernel_x_3/2) + np.floor(kernel_x_4/2) + np.floor(kernel_last_x/2))) * Rx + ind_acc_x - 1;
                #print ('target_x_start, target_x_end: ', target_x_start, target_x_end)
                for ind_acc_y in range(Ry):
                    target_y_start = np.int32((np.ceil(kernel_y_1/2)-1) + (np.ceil(kernel_y_2/2)-1) + (np.ceil(kernel_y_3/2)-1) + (np.ceil(kernel_y_4/2)-1) + (np.ceil(kernel_last_y/2)-1)) * Ry + ind_acc_y
                    target_y_end = ACS_out_dim_Y  - np.int32((np.floor(kernel_y_1/2) + np.floor(kernel_y_2/2) + np.floor(kernel_y_3/2) + np.floor(kernel_y_4/2) + np.floor(kernel_last_y/2))) * Ry + ind_acc_y - 1;
                    #print ('target_y_start, target_y_end: ', target_y_start, target_y_end)
                    #print (ind_acc_y+ind_acc_x*Ry+ind_c*Ry*Rx)

                    target[0,:,:,:,ind_acc_y+ind_acc_x*Ry+ind_c*Ry*Rx] = ACS_out[0,target_x_start:target_x_end + 1, target_y_start:target_y_end +1,target_t_start:target_t_end + 1,ind_c];
    ...
    res = cnn_3layer(kspace_und_re_sq,w1,b1,w2,b2,w3,b3,w4,w5,1,1,sess)
    ...
    target_t_start = np.int32((np.ceil(kernel_t_1/2)-1) + (np.ceil(kernel_t_2/2)-1) + (np.ceil(kernel_t_3/2)-1) + (np.ceil(kernel_t_4/2)-1) + (np.ceil(kernel_last_t/2)-1)) * 1;
    target_t_end = dim_kspaceUnd_T  - np.int32((np.floor(kernel_t_1/2) + np.floor(kernel_t_2/2) + np.floor(kernel_t_3/2) + np.floor(kernel_t_4/2) + np.floor(kernel_last_t/2))) * 1 -1;
    ...
    for ind_c in range(0,no_channels):
        ...
        for ind_acc_x in range(Rx):
            ...
            target_x_start = np.int32((np.ceil(kernel_x_1/2)-1) + (np.ceil(kernel_x_2/2)-1) + (np.ceil(kernel_x_3/2)-1) + (np.ceil(kernel_x_4/2)-1) + (np.ceil(kernel_last_x/2)-1)) * Rx + ind_acc_x;
            target_x_end = dim_kspaceUnd_X  - np.int32((np.floor(kernel_x_1/2) + np.floor(kernel_x_2/2) + np.floor(kernel_x_3/2) + np.floor(kernel_x_4/2) + np.floor(kernel_last_x/2))) * Rx + ind_acc_x - 1;

            for ind_acc_y in range(Ry):
                target_y_start = np.int32((np.ceil(kernel_y_1/2)-1) + (np.ceil(kernel_y_2/2)-1) + (np.ceil(kernel_y_3/2)-1) + (np.ceil(kernel_y_4/2)-1) + (np.ceil(kernel_last_y/2)-1)) * Ry + ind_acc_y
                target_y_end = dim_kspaceUnd_Y  - np.int32((np.floor(kernel_y_1/2) + np.floor(kernel_y_2/2) + np.floor(kernel_y_3/2) + np.floor(kernel_y_4/2) + np.floor(kernel_last_y/2))) * Ry + ind_acc_y - 1;

                kspace_recon[0,target_x_start:target_x_end + 1:Rx, target_y_start:target_y_end +1:Ry,target_t_start:target_t_end + 1,ind_c] = res[0,:,:,:,ind_acc_y+ind_acc_x*Ry+ind_c*Ry*Rx]
