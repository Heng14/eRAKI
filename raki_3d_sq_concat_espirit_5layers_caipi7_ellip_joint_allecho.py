...
def raki(num_chan, N1, N2, Nt, acs_start_index_x, acs_end_index_x, acs_start_index_y, acs_end_index_y, acs_start_index_t, acs_end_index_t, kspace_undersampled_zero_filled, kspace_acs_zero_filled, kspace_coils_acs_out, kspace_acs_cropped, Rx, Ry, debug_path, name_image, name_weight,train_flag=True, finetune=False, slice_n=0):
    ...
    kspace_und_out_re = np.zeros([dim_kspaceUnd_X,dim_kspaceUnd_Y,dim_kspaceUnd_T,ACS_out_dim_Z])
    ...
    kspace_und_out_re = np.reshape(kspace_und_out_re,[1,dim_kspaceUnd_X,dim_kspaceUnd_Y,dim_kspaceUnd_T,ACS_out_dim_Z])
...
def show_raki_results_3d(image_original0, debug_path0, name_image):
    ...
    image_original_all = np.transpose(image_original0,(3,0,1,2))
    ...
    #echo = 2
    #image_original0 = np.abs(image_original_all)[echo]
    #raki_rssq_image_sub0 = np.abs(coil_images_raki_sub)[echo]
    tmp = np.abs(image_original_all)[0]
    print (np.max(tmp))

    echo = 123
    image_original0 = np.sqrt(np.sum(np.square(np.abs(image_original_all)),axis=0))
    raki_rssq_image_sub0 = np.sqrt(np.sum(np.square(np.abs(coil_images_raki_sub)),axis=0))


    print (np.max(image_original0))
    raise
    ...
    debug_path = os.path.join(debug_path0, f'3d_betmask_echo{echo}_{rmse_str}')
    os.makedirs(debug_path, exist_ok=True)

    debug_path_ssim = os.path.join(debug_path0, f'3d_betmask_ssim_echo{echo}_{ssim_e_str}')
